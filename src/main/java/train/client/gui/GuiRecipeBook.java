...
public class GuiRecipeBook extends GuiScreen {
	...
	public ArrayList<ArrayList<StackToDraw>> leftPageItemStacks = new ArrayList<>();
	public ArrayList<String> rightPage = new ArrayList<String>();
	public ArrayList<String> rightPageImage = new ArrayList<String>();
	public ArrayList<ArrayList<StackToDraw>> rightPageItemStacks = new ArrayList<>();
	private List<ITCRecipe> recipeListWB =null;
    ...
    public GuiRecipeBook(EntityPlayer par1EntityPlayer, ItemStack par2ItemStack) {
        ...
        this.currPage = itemstackBook.getTagCompound().getInteger("currPage");
        this.currRecipe = itemstackBook.getTagCompound().getInteger("currRecipe");
        ...
        addPage("Welcome to the Traincraft guide! \nThis book contains everything you need to know about Traincraft.\n\nAuthors:\nSpitfire4466,\nMrbrutal\n\nBukkit port: \nDV8FromTheWorld\n\nThanks to CovertJaguar for his help and a great API.\n", "", "left", new ArrayList<StackToDraw>() {
            {
                // TODO: figure out a way to re-add this that doesn't require hard-coding names
		...
		if (rightPage != null && recipeList != null && recipeListWB != null)
			bookTotalPages = rightPage.size() + (recipeList.size() / 2) + (recipeListWB.size() / 2);
    ...
    public void initGui() {
        ...
        searchBar = new GuiTextField(fontRendererObj, halfWidth + 23, ((halfHeight - bookImageHeight / 2) - 9), 100, 10);
		searchBar.setEnableBackgroundDrawing(false);
		searchBar.setFocused(true);
		searchBar.setCanLoseFocus(false);
		searchBar.setMaxStringLength(25);
		buttonList.add(buttonBack = new GuiButtonNextPage(4, halfWidth + 150, halfHeight + 80, 23, 13, true));
        buttonList.add(buttonRead = new GuiButtonNextPage(3, halfWidth - 8, halfHeight + 98, 40, 20, true));
        buttonList.add(buttonNextPage = new GuiButtonNextPage(1, halfWidth + 150, halfHeight + 80, 23, 13, true));
        buttonList.add(buttonPreviousPage = new GuiButtonNextPage(2, halfWidth - 180, halfHeight + 80, 23, 13, false));
		buttonList.add(buttonSearchPrevious = new GuiButtonRecipeSearch(5, halfWidth + 160, ((halfHeight - bookImageHeight / 2) - 10), 10, 10, GuiButtonRecipeSearch.Type.PREVIOUSRESULT));
		buttonList.add(buttonSearchNext = new GuiButtonRecipeSearch(6, halfWidth + 170, ((halfHeight - bookImageHeight / 2) - 10), 10, 10,  GuiButtonRecipeSearch.Type.NEXTRESULT));
		buttonList.add(buttonSearch = new GuiButtonRecipeSearch(7, halfWidth + 7, ((halfHeight - bookImageHeight / 2) - 10), 10, 10, GuiButtonRecipeSearch.Type.SEARCH));
        updateButtons();
    }
    ...
    private void updateButtons() {
		...
		buttonBack.visible = (currPage == bookTotalPages-1);
		buttonBack.showButton = true;
		buttonRead.visible = (currPage == 0);
		buttonRead.showButton = false;
		buttonNextPage.visible = (currPage > 0 && currPage < bookTotalPages - 1);
		buttonNextPage.showButton = (currPage > 0 && currPage < bookTotalPages - 1);
		buttonPreviousPage.visible = currPage > 0;
		buttonPreviousPage.showButton = currPage > 0;
		buttonSearchPrevious.visible = currPage > 0;
		buttonSearchPrevious.showButton = currPage > 0;
		buttonSearchNext.visible = currPage > 0;
		buttonSearchNext.showButton = currPage > 0;
		buttonSearch.visible = currPage > 0;
		buttonSearch.showButton = currPage > 0;
	}
	...
	public void drawScreen(int par1, int par2, float par3) {
        ...
        int var6 = (height) / 2 - bookImageHeight / 2;
		...
		if (currPage > 0) {
			...
			drawTexturedModalRect(var5, var6, 0, 0, bookImageWidth, bookImageHeight + 20);
			mc.renderEngine.bindTexture(new ResourceLocation(Info.resourceLocation,Info.bookPrefix + "searchbar.png"));
			drawTexturedModalRect(SEARCH_BOX_X, SEARCH_BOX_Y, 0, 0, 192, 13);
			mc.renderEngine.bindTexture(new ResourceLocation(Info.resourceLocation,Info.bookPrefix + "bookleft.png"));
			var5 -= bookImageWidth;
			drawTexturedModalRect(var5, var6, 256 - bookImageWidth, 0, bookImageWidth, bookImageHeight);
			searchBar.drawTextBox();
		}
		else {
			mc.renderEngine.bindTexture(new ResourceLocation(Info.resourceLocation,Info.bookPrefix + "bookcover.png"));
			drawTexturedModalRect(var5 - 55, var6 - 15, 0, 0, 256, 256);
		}

        pageIndic = String.format(StatCollector.translateToLocal("book.pageIndicator"), new Object[]{currPage + 1, bookTotalPages});

		var9 = fontRendererObj.getStringWidth(pageIndic);
		if (currPage > 0) {
			fontRendererObj.drawString(pageIndic, var5 - var9 + bookImageWidth - 44, var6 + 7, 0);
			fontRendererObj.drawString(fontRendererObj.trimStringToWidth(searchBar.getText(), SEARCH_BOX_TEXT_MAX_WIDTH), SEARCH_BOX_X + 21, SEARCH_BOX_Y + 3, 0);
		}
		super.drawScreen(par1, par2, par3);

        if (currPage < rightPage.size()) {
            fontRendererObj.drawSplitString(leftPage.get(currPage), var5 + 36, var6 + 16 + 16, 140, 0);

            fontRendererObj.drawSplitString(rightPage.get(currPage), var5 + 250, var6 + 16 + 16, 140, 0);

            GL11.glEnable(32826);
            RenderHelper.enableGUIStandardItemLighting();
            if (leftPageItemStacks.size() >= currPage && leftPageItemStacks.get(currPage) != null) {
                for (StackToDraw stack : leftPageItemStacks.get(currPage)) {
                    if (stack != null) {
                        renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, stack.getItemStack(), var5 +stack.getX(), var6 +stack.getY());
                    }
                }
            }

            if (rightPageItemStacks.size() >= currPage && rightPageItemStacks.get(currPage) != null) {
                for (StackToDraw stack : rightPageItemStacks.get(currPage)) {
                    if (stack != null) {
                        renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, stack.getItemStack(), var5 + stack.getX() + 210, var6 + stack.getY());
                    }
                }
            }
            GL11.glDisable(32826);
        }
        if (currPage > rightPage.size() - 1) {
            //System.out.println((rightPage.size()*2) -1);
            int page = currRecipe - (rightPage.size() * 2) + 1;
            // Drawing the non-train recipes...
            if (!(page > recipeListWB.size() - 1)) {
                drawWorkBenchBackground(var5, var6, 0, var9, "right");
                drawWorkBenchBackground(var5, var6, 0, var9, "left");
                RenderHelper.enableGUIStandardItemLighting();
    ...
    private void drawAssemblyBackground(List<TierRecipe> recipeList, int var5, int var6, int page, int var9, String side) {
        ...
        if (side.equals("left")) drawTexturedModalRect(var5 + 70, var6 + 50, 0, 0, 177, 163);
        if (side.equals("right")) drawTexturedModalRect(var5 + 340, var6 + 50, 0, 0, 177, 163);
    }

    private void drawWorkBenchBackground(int var5, int var6, int page, int var9, String side) {
        //int var4 = mc.renderEngine.getTexture("/gui/crafting.png");
        //GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
        mc.renderEngine.bindTexture(new ResourceLocation(Info.resourceLocation, Info.guiPrefix + "crafting_table.png"));
        if (side.equals("left")) drawTexturedModalRect(var5 + 20, var6 + 50, 0, 0, 177, 80);
        if (side.equals("right")) drawTexturedModalRect(var5 + 215, var6 + 50, 0, 0, 177, 80);
    }

    private void drawWorkBenchRecipe(List<ITCRecipe> recipeList, int var5, int var6, int page, int var9, String side) {
        ...
        GL11.glEnable(32826);
        if (itemList[0] != null) {
            renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, itemList[0], var5 + 50 + offset, var6 + 67);
        }

        if (itemList[1] != null) {
            renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, itemList[1], var5 + 68 + offset, var6 + 67);
        }

        if (itemList[2] != null) {
            renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, itemList[2], var5 + 86 + offset, var6 + 67);
        }

        if (itemList[3] != null) {
            renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, itemList[3], var5 + 50 + offset, var6 + 85);
        }

        if (itemList[4] != null) {
            renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, itemList[4], var5 + 68 + offset, var6 + 85);
        }

        if (itemList[5] != null) {
            renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, itemList[5], var5 + 86 + offset, var6 + 85);
        }

        if (itemList[6] != null) {
            renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, itemList[6], var5 + 50 + offset, var6 + 103);
        }
        if (itemList[7] != null) {
            renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, itemList[7], var5 + 68 + offset, var6 + 103);
        }

        if (itemList[8] != null) {
            renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, itemList[8], var5 + 86 + offset, var6 + 103);
        }

        if (itemOutput != null && itemOutput.getItem() != null) {
            renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, itemOutput, var5 + 145 + offset, var6 + 85);
            // Draw name of recipe. Highlight in green if it contains the search query.
            fontRendererObj.drawString(itemOutput.getItem().getItemStackDisplayName(itemOutput), var5 + 20 + offset, var6 + 40, (!searchBar.getText().isEmpty() && itemOutput.getItem().getItemStackDisplayName(itemOutput).toLowerCase().contains(searchBar.getText().toLowerCase())) ? 0x21d12d : 0);
        }

        if (itemOutput != null) {
            fontRendererObj.drawString("Crafted in: Train Workbench", var5 + 20 + offset, var6 + 130, 0);
            ...
            for (int z = 0; z < RecipeBookHandler.vanillaWorkTableRecipes.length; z++) {
                if (itemOutput.getItem() != null && RecipeBookHandler.vanillaWorkTableRecipes[z] != null && RecipeBookHandler.vanillaWorkTableRecipes[z].equals(itemOutput.getItem().getItemStackDisplayName(itemOutput))) {
                    fontRendererObj.drawString("Also crafted in: Crafting Table", var5 + 20 + offset, var6 + 140, 0);
                    break;
                }
            }
        }
    ...
    private void drawAssemblyRecipe(List<TierRecipe> recipeList, int var5, int var6, int page, int var9, String side) {
        ...
        if (itemList.get(0) != null) {
            renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, itemList.get(0), var5 + 94 + offset, var6 + 76);
        }

        if (itemList.get(0) != null) {
            renderItem.renderItemOverlayIntoGUI(fontRendererObj, mc.renderEngine, itemList.get(0), var5 + 94 + offset, var6 + 76);
        }

        if (itemList.get(1) != null) {
            renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, itemList.get(1), var5 + 113 + offset, var6 + 143);
        }

        if (itemList.get(1) != null) {
            renderItem.renderItemOverlayIntoGUI(fontRendererObj, mc.renderEngine, itemList.get(1), var5 + 113 + offset, var6 + 143);
        }
        if (itemList.get(2) != null) {
            renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, itemList.get(2), var5 + 148 + offset, var6 + 143);
        }

        if (itemList.get(2) != null) {
            renderItem.renderItemOverlayIntoGUI(fontRendererObj, mc.renderEngine, itemList.get(2), var5 + 148 + offset, var6 + 143);
        }

        if (itemList.get(3) != null) {
            renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, itemList.get(3), var5 + 214 + offset, var6 + 143);
        }

        if (itemList.get(3) != null) {
            renderItem.renderItemOverlayIntoGUI(fontRendererObj, mc.renderEngine, itemList.get(3), var5 + 214 + offset, var6 + 143);
        }

        if (itemList.get(4) != null) {
            renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, itemList.get(4), var5 + 148 + offset, var6 + 77);
        }

        if (itemList.get(4) != null) {
            renderItem.renderItemOverlayIntoGUI(fontRendererObj, mc.renderEngine, itemList.get(4), var5 + 148 + offset, var6 + 77);
        }

        if (itemList.get(5) != null) {
            renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, itemList.get(5), var5 + 184 + offset, var6 + 77);
        }

        if (itemList.get(5) != null) {
            renderItem.renderItemOverlayIntoGUI(fontRendererObj, mc.renderEngine, itemList.get(5), var5 + 184 + offset, var6 + 77);
        }

        if (itemList.get(6) != null) {
            renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, itemList.get(6), var5 + 149 + offset, var6 + 110);
        }

        if (itemList.get(6) != null) {
            renderItem.renderItemOverlayIntoGUI(fontRendererObj, mc.renderEngine, itemList.get(6), var5 + 149 + offset, var6 + 110);
        }

        if (itemList.get(7) != null) {
            renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, itemList.get(7), var5 + 185 + offset, var6 + 110);
        }

        if (itemList.get(7) != null) {
            renderItem.renderItemOverlayIntoGUI(fontRendererObj, mc.renderEngine, itemList.get(7), var5 + 185 + offset, var6 + 110);
        }

        if (itemList.get(8) != null) {
            renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, itemList.get(8), var5 + 94 + offset, var6 + 110);
        }

        if (itemList.get(8) != null) {
            renderItem.renderItemOverlayIntoGUI(fontRendererObj, mc.renderEngine, itemList.get(8), var5 + 94 + offset, var6 + 110);
        }

        if (itemList.get(9) != null) {
            renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, itemList.get(9), var5 + 214 + offset, var6 + 77);
        }

        if (itemList.get(9) != null) {
            renderItem.renderItemOverlayIntoGUI(fontRendererObj, mc.renderEngine, itemList.get(9), var5 + 214 + offset, var6 + 77);
        }

        ItemStack output = recipeList.get(page).getOutput();
        if (output != null) {
            if (side.equals("left")) {
                renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, output, var5 + 162, var6 + 177);
            }

            if (side.equals("right")) {
                renderItem.renderItemIntoGUI(fontRendererObj, mc.renderEngine, output, var5 + 432, var6 + 177);
            }
        }
        ...
        if (side.equals("left")) {
            fontRendererObj.drawString("Tier: " + tier, var5 - var9 + bookImageWidth - 56, var6 + 40, 0);
            fontRendererObj.drawString(fontRendererObj.trimStringToWidth(name, 150), var5 - var9 + bookImageWidth - 45, var6 + 56, (!searchBar.getText().isEmpty() && drawColorHighlightFlag) ? 0x21d12d : 0xffffff);
        }

        if (side.equals("right")) {
            fontRendererObj.drawString(fontRendererObj.trimStringToWidth(name, 150), var5 - var9 + bookImageWidth + 225, var6 + 56, (!searchBar.getText().isEmpty() && drawColorHighlightFlag) ? 0x21d12d : 0xffffff);
            fontRendererObj.drawString("Tier: " + tier, var5 - var9 + bookImageWidth + 338, var6 + 40, 0);
        }
        GL11.glDisable(32826);
    }
    ...
    public void onGuiClosed() {
        ItemRecipeBook.page = currPage;
        ItemRecipeBook.recipe = currRecipe;
        itemstackBook.getTagCompound().setInteger("currPage", currPage);
        itemstackBook.getTagCompound().setInteger("currRecipe", currRecipe);
        super.onGuiClosed();
    }
	...
	protected void keyTyped(char eventChar, int eventKey) {
		...
		if (eventChar == '\r') { // If character is return...
			...
			searchBar.textboxKeyTyped(eventChar, eventKey);
			resetSearch();
		}
	}
	...
	public void runSearch() {
		...
		if (!resultIndexList.isEmpty() && searchResultsPosition < resultIndexList.size()) { // If the search is complete, display and increment result.
			if (searchResultsPosition == resultIndexList.size() - 1)
				buttonSearchNext.setType(GuiButtonRecipeSearch.Type.NEXTRESULT, GuiButtonRecipeSearch.Texture.INACTIVE);
			else
				buttonSearchNext.setType(GuiButtonRecipeSearch.Type.NEXTRESULT, GuiButtonRecipeSearch.Texture.ACTIVE);
			if (searchResultsPosition - 1 < 0)
				buttonSearchPrevious.setType(GuiButtonRecipeSearch.Type.PREVIOUSRESULT, GuiButtonRecipeSearch.Texture.INACTIVE);
			else
				buttonSearchPrevious.setType(GuiButtonRecipeSearch.Type.PREVIOUSRESULT, GuiButtonRecipeSearch.Texture.ACTIVE);
			currPage = resultIndexList.get(searchResultsPosition);
			currRecipe = currPage * 2;
			updateButtons();
		}
	}
	...
	private void initializeRecipeSearchMap() {
		...
		for (int i = 0; i < recipeListWB.size(); i++) {
			name = recipeListWB.get(i).getRecipeOutput().getDisplayName().toLowerCase();
