...
public class HUDloco extends GuiScreen {
    ...
    public void onGameRender(RenderGameOverlayEvent.Text event)
    {
        if (game != null && game.thePlayer != null && game.thePlayer.ridingEntity != null && Minecraft.isGuiEnabled() && game.currentScreen == null
                && (game.thePlayer.ridingEntity instanceof Locomotive
                    || (game.thePlayer.ridingEntity instanceof EntitySeat && ((EntitySeat) game.thePlayer.ridingEntity).getHost() instanceof Locomotive)))
        {
            if (game.thePlayer.ridingEntity instanceof EntitySeat)
            {
                ...
                {
                    renderSkillHUD(event, (Locomotive) ((EntitySeat) game.thePlayer.ridingEntity).getHost());
                }
            }
            else
            {
    ...
    public void renderSkillHUD(RenderGameOverlayEvent event, Locomotive rcCar) {
        ...
        // Render Water Bar because the steam train has water.
        if (rcCar instanceof SteamTrain) {
            renderWaterBar(rcCar);
        }

        // Some locos may not overheat in the future.
    ...
    private void renderFuelBar(Locomotive loco) {
        ...
        // So that the content of the tank is rendered and not the fuel current burned.
        ...
        // Steam trains have a different HUD.
        if (loco instanceof SteamTrain) {
            game.renderEngine.bindTexture(new ResourceLocation(Info.resourceLocation, Info.guiPrefix + "loco_hud_steam.png"));
        } else {
            game.renderEngine.bindTexture(new ResourceLocation(Info.resourceLocation, Info.guiPrefix + "locohud.png"));
        ...
        // Things are slightly different in the Steam HUD.
    ...
    private void renderWaterBar(Locomotive loco) {
        ...
        // Scaled to fit the 49 pixel bar.
        if (l_Scaled > 49) {
            l_Scaled = 49;
        }
        ...
        // Because it's a black bar that is rendered that hides the color bar hte black bar is rendered from top to bottom.
        GL11.glEnable(3042 /* GL_BLEND */);
        GL11.glEnable(32826);
    ...
    private void renderSpeedometer(Locomotive loco) {
        ...
        // SteamTrain has a different HUD.
        if (loco instanceof SteamTrain) {
            game.renderEngine.bindTexture(new ResourceLocation(Info.resourceLocation, Info.guiPrefix + "loco_hud_steam.png"));
        } else {
            game.renderEngine.bindTexture(new ResourceLocation(Info.resourceLocation, Info.guiPrefix + "locohud.png"));
